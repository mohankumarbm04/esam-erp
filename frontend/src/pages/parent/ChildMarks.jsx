      ...
      <main className="max-w-7xl mx-auto py-6 px-4">
        ...
        <div className="mt-6 bg-blue-50 rounded-xl p-4">
          <p className="text-sm text-blue-700">
            <span className="font-medium">📊 Performance Insight:</span> Overall
            SGPA of {summary.sgpa} is above the departmental average.
            {marksData.filter((s) => s.rank && s.rank <= 5).length > 0 &&
              ` Ranked in top 5 in ${marksData.filter((s) => s.rank && s.rank <= 5).length} subjects.`}
          </p>
        </div>
      </main>
    </div>
